...
export function weekendResponsibilityContentPlugin(): Plugin {
  return {
    ...
    transform(source, id) {
      ...
      const friZhNew = "tasksZh: ['11:30–14:00 正常出品，同时边出茶边按目标补料。','当天营业使用的 Coconut Premix 由早班现做并当天用完；不得使用隔夜 Coconut，也不得提前为第二天制作。','高峰前检查 Black Tea：楼下备用量 + 茶桶现有量一起算；合计低于 4L 时，立即泡约 8L 新 Black Tea；合计达到 4L 或以上则不用再泡。','14:00 后中班到岗，早班转为专注补料，不再主要负责出品。','16:00 前完成 FRI 14:00 目标中的所有物料。','在管理小程序提交实际完成数量。','补回自己使用过的原料到指定储藏位置。'],";
      ...
      const satEnNew = "tasksEn: ['Check Friday prep: freshness, expiry time, and labels.','Move items into tea buckets and topping boxes so they are ready for service.','Refill front counter and drawer ingredients.','Make sure milk, canned items, sparkling water and toppings are ready to use.','Submit actual completed quantities in the management app.','Before 13:30, count the creamer containers. Make new creamer only when there are fewer than 2 containers. A container currently in use counts as one; the containers do not need to be full.','Saturday Coconut Premix must be made fresh by the opening shift and used the same day. Before 13:30, make sure there are 6L of today’s fresh Coconut; never use overnight Coconut, and make more fresh Coconut if below 6L.','Before peak, combine the Black Tea downstairs reserve and the tea currently in the service bucket. If the combined total is below 4L, brew about 8L of fresh Black Tea immediately. If the total is 4L or more, do not brew another batch.'],";
      ...
      const sunZhNew = "tasksZh: ['检查周六准备物料是否可用：新鲜度、效期、标签。','将物料倒入茶桶和料盒，确保可直接出品。','当天营业使用的 Coconut Premix 由早班现做并当天用完；不得使用隔夜 Coconut，也不得提前为第二天制作。','高峰前检查 Black Tea：楼下备用量 + 茶桶现有量一起算；合计低于 4L 时，立即泡约 8L 新 Black Tea；合计达到 4L 或以上则不用再泡。','严格先进先出：前旧后新，先用旧料。','补齐前场和抽屉原料。','确认牛奶、罐头、气泡水和小料可直接使用。'],";
