...
export function weekendResponsibilityContentPlugin(): Plugin {
  return {
    ...
    transform(source, id) {
      ...
      const friZhNew = "tasksZh: ['11:30–14:00 正常出品，同时边出茶边按目标补料。','当天营业使用的 Coconut Premix 由早班现做并当天用完；不得使用隔夜 Coconut，也不得提前为第二天制作。','高峰前检查 Black Tea：奶茶机下面冰箱里的 Black Tea + 茶桶现有量一起算；合计低于 4L 时，立即泡约 8L 新 Black Tea。','14:00 后中班到岗，早班转为专注补料，不再主要负责出品。','16:00 前完成 FRI 14:00 目标中的所有物料。','在管理小程序提交实际完成数量。','补回自己使用过的原料到指定储藏位置。'],";
      ...
      const friEnNewGate = "tasksEn: ['11:30–14:00: make drinks and prep according to the target list at the same time.','The opening shift must make the Coconut Premix for that day fresh and use it the same day. Do not use overnight Coconut or prepare it in advance for the next day.','Before peak, combine the Black Tea in the fridge under the milk tea machine with the tea currently in the service bucket. If the combined total is below 4L, brew about 8L of fresh Black Tea immediately.','After 14:00, once support arrives, focus on prep instead of drink production.','Finish all FRI 14:00 target items before 16:00.','Submit the actual completed quantities in the management app.','Refill and return all ingredients you used to their assigned storage places.'],";
      ...
      const satZhNew = "tasksZh: ['检查周五准备的物料是否可用：新鲜度、效期、标签。','将物料倒入茶桶和料盒，确保可直接出品。','补齐前场和抽屉原料。','确认牛奶、罐头、气泡水和小料都可直接使用。','在管理小程序提交实际完成数量。','13:30 前检查奶精 container 数量；低于 2 个 container 才泡新的奶精，正在使用中的 container 也算 1 个。','周六当天 Coconut Premix 必须由早班现做并当天用完；13:30 前确保当日新做 Coconut 有 6L，不得使用隔夜料，低于 6L 时补足。','高峰前检查 Black Tea：奶茶机下面冰箱里的 Black Tea + 茶桶现有量一起算；合计低于 4L 时，立即泡约 8L 新 Black Tea。'],";
      ...
      const satEnNew = "tasksEn: ['Check Friday prep: freshness, expiry time, and labels.','Move items into tea buckets and topping boxes so they are ready for service.','Refill front counter and drawer ingredients.','Make sure milk, canned items, sparkling water and toppings are ready to use.','Submit actual completed quantities in the management app.','Before 13:30, count the creamer containers. Make new creamer only when there are fewer than 2 containers. A container currently in use counts as one.','Saturday Coconut Premix must be made fresh by the opening shift and used the same day. Before 13:30, make sure there are 6L of today’s fresh Coconut; never use overnight Coconut, and make more fresh Coconut if below 6L.','Before peak, combine the Black Tea in the fridge under the milk tea machine with the tea currently in the service bucket. If the combined total is below 4L, brew about 8L of fresh Black Tea immediately.'],";
      ...
      const sunZhNew = "tasksZh: ['检查周六准备物料是否可用：新鲜度、效期、标签。','将物料倒入茶桶和料盒，确保可直接出品。','当天营业使用的 Coconut Premix 由早班现做并当天用完；不得使用隔夜 Coconut，也不得提前为第二天制作。','高峰前检查 Black Tea：奶茶机下面冰箱里的 Black Tea + 茶桶现有量一起算；合计低于 4L 时，立即泡约 8L 新 Black Tea。','严格先进先出：前旧后新，先用旧料。','补齐前场和抽屉原料。','确认牛奶、罐头、气泡水和小料可直接使用。'],";
      ...
      const sunEnNew = "tasksEn: ['Check Saturday prep: freshness, expiry time, and labels.','Move items into tea buckets and topping boxes for direct service.','The opening shift must make the Coconut Premix for that day fresh and use it the same day. Do not use overnight Coconut or prepare it in advance for the next day.','Before peak, combine the Black Tea in the fridge under the milk tea machine with the tea currently in the service bucket. If the combined total is below 4L, brew about 8L of fresh Black Tea immediately.','Strict FIFO: old stock in front, new stock behind, use old stock first.','Refill front counter and drawer ingredients.','Make sure milk, canned items, sparkling water and toppings are ready to use.'],";
